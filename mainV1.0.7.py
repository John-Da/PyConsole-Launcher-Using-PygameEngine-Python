...
from themes.system_theme import SystemTheme, THEMES
...
screen = pygame.display.set_mode(
    (WIDTH, HEIGHT), pygame.FULLSCREEN | pygame.SCALED | pygame.RESIZABLE
)
...
def launch_game(game_data, current_theme):
    ...
    draw_loading_screen(
        ...
        font_main,
        current_theme,
        f"BOOTING {game_data['name'].upper()}...",
    )
    ...
    try:
        ...
        screen = pygame.display.set_mode(
            (W, H), pygame.FULLSCREEN | pygame.SCALED | pygame.RESIZABLE
        )
...
while running:
    ...
    SIDE_M = 30
    cols = (
        max(1, int((W - (SIDE_M * 2)) // (box_size + grid_gutter)))
        if view_mode == "grid"
        else 1
    )
    ...
    search_bar_rect = pygame.Rect(
        add_rect.right + GUTTER, 95, exit_rect.right - (add_rect.right + GUTTER), BTN_H
    )
    search_btn_rect = pygame.Rect(
        search_bar_rect.x + 5, search_bar_rect.y + 5, 75, BTN_H - 10
    )
    ...
    if input_manager.last_input in ("mouse", "touch"):
        ...
        if not (
            theme_rect.collidepoint(m_pos)
            or grid_rect.collidepoint(m_pos)
            or list_rect.collidepoint(m_pos)
            or exit_rect.collidepoint(m_pos)
            or add_rect.collidepoint(m_pos)
            or search_btn_rect.collidepoint(m_pos)
        ):
    ...
    if vk.active:
        ...
        if input_manager.actions["ACCEPT"]:
            if search_bar_rect.collidepoint(m_pos) or (
                focus_mode == "buttons" and button_selected == 5
            ):
                vk.active = True
        ...
        if input_manager.actions["ACCEPT"]:
            ...
            if theme_rect.collidepoint(m_pos) or (
                focus_mode == "top_bar" and button_selected == 1
            ):
            ...
            elif grid_rect.collidepoint(m_pos) or (
                focus_mode == "top_bar" and button_selected == 2
            ):
                view_mode = "grid"
                save_settings(
                    SETTINGS_FILE, LIBRARY_FOLDER, current_theme_idx, view_mode
            ...
            elif list_rect.collidepoint(m_pos) or (
                focus_mode == "top_bar" and button_selected == 3
            ):
                view_mode = "list"
                save_settings(
                    SETTINGS_FILE, LIBRARY_FOLDER, current_theme_idx, view_mode
            ...
            elif exit_rect.collidepoint(m_pos) or (
                focus_mode == "top_bar" and button_selected == 4
            ):
                save_settings(
                    SETTINGS_FILE, LIBRARY_FOLDER, current_theme_idx, view_mode
                )
                draw_loading_screen(
                    screen, clock, font_main, theme, f"CLOSING {APP_NAME.upper()}..."
                )
            ...
            elif add_rect.collidepoint(m_pos) or (
                ...
                if p:
                    LIBRARY_FOLDER = p
                    games = load_games(p)
            elif search_bar_rect.collidepoint(m_pos) or (
                focus_mode == "buttons" and button_selected == 5
            ):
                focus_mode = "search"
            ...
            for i, g in enumerate(filtered):
                ...
                if -250 < y < H + 250:
                    ...
                    if is_sel:
                        if rect.bottom > H - 50:
                            target_scroll_y += 10
                        if rect.top < HEADER_H + 50:
                            target_scroll_y -= 10

                    draw_round_rect(screen, theme["header"], rect, 15)
                    if is_sel:
                        draw_round_rect(
                            screen, theme["accent"], rect.inflate(10, 10), 18, 4
                        )

                    if g["icon"]:
                        img = pygame.transform.smoothscale(
                            g["icon"], (rect.width - 60, rect.height - 60)
                        )
            ...
            for i, g in enumerate(filtered):
                ...
                if -100 < y < H + 100:
                    ...
                    if is_sel:
                        if rect.bottom > H - 20:
                            target_scroll_y += 10
                        if rect.top < HEADER_H + 20:
                            target_scroll_y -= 10

                    draw_round_rect(screen, theme["header"], rect, 12)
                    if is_sel:
                        draw_round_rect(screen, theme["accent"], rect, 12, 3)

                    if g["icon"]:
                        img = pygame.transform.smoothscale(g["icon"], (60, 60))
                        screen.blit(img, (rect.x + 15, rect.y + 12))
                    screen.blit(
                        font_main.render(g["name"], True, theme["text"]),
                        (rect.x + 90, rect.y + 15),
                    )
                    screen.blit(
                        font_meta.render(
                            f"by {g['author']}  |  v{g['version']}", True, theme["text"]
                        ),
                        (rect.x + 90, rect.y + 40),
                    )
                    ...
                    screen.blit(
                        font_ui.render(size_info, True, theme["accent"]),
                        (rect.x + 90, rect.y + 58),
                    )
    ...
    pygame.draw.rect(
        screen, theme["accent"], (SIDE_M + 5, title_surf.get_height() + 65, 190, 3)
    )
    ...
    stxt = (
        search_query + ("|" if focus_mode == "search" and time.time() % 1 > 0.5 else "")
        if search_query or focus_mode == "search"
        else "Search library..."
    )
    screen.blit(
        font_main.render(stxt, True, theme["text"]),
        (search_btn_rect.right + 15, search_bar_rect.y + 10),
    )
