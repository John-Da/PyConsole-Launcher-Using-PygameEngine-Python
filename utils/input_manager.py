...
class InputManager:
    def __init__(self, joysticks):
        self.actions = {
            ...
            "ACCEPT": False, "BACK": False, "SCROLL": 0,
            "MOUSE_POS": (0, 0), "CLICK": False, "ANY_KEY": None
        }
        self.joy_delay = 0
    ...
    def reset_frame(self):
        for k in ["UP", "DOWN", "LEFT", "RIGHT", "ACCEPT", "BACK", "CLICK"]:
    ...
    def process_events(self, W, H):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_UP: self.actions["UP"] = True
                if event.key == pygame.K_DOWN: self.actions["DOWN"] = True
                if event.key == pygame.K_LEFT: self.actions["LEFT"] = True
                if event.key == pygame.K_RIGHT: self.actions["RIGHT"] = True
                if event.key in (pygame.K_RETURN, pygame.K_SPACE): self.actions["ACCEPT"] = True
                if event.key == pygame.K_ESCAPE: self.actions["BACK"] = True
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                if event.button == 0: self.actions["ACCEPT"] = True
                if event.button == 1: self.actions["BACK"] = True

            if event.type == pygame.JOYDEVICEADDED:
                joy = pygame.joystick.Joystick(event.device_index)
                self.joysticks[joy.get_instance_id()] = joy
            if event.type == pygame.JOYDEVICEREMOVED:
                if event.instance_id in self.joysticks: del self.joysticks[event.instance_id]
        ...
        if self.joysticks and current_time > self.joy_delay:
            ...
            for joy in self.joysticks.values():
                ...
                if ax_y < -0.5 or hat_y > 0.5:
                    self.actions["UP"] = True; moved = True
                elif ax_y > 0.5 or hat_y < -0.5:
                    self.actions["DOWN"] = True; moved = True
                elif ax_x < -0.5 or hat_x < -0.5:
                    self.actions["LEFT"] = True; moved = True
                elif ax_x > 0.5 or hat_x > 0.5:
                    self.actions["RIGHT"] = True; moved = True
            if moved:
                self.last_input = "controller"
